...
.align 2
prompt6: .ascii "What size pizza would you like to order?(s/m/l) \0"
.align 2
string:	.ascii  " %c\0"
	.align 2
format: .ascii "%d\0"
.align 2
prompt7: .ascii "Would you like to order another pizza? (y/n)\n\0"
.align 2
...
main:
	...
	ldr r0, prompt6_ptr /*string for what pizza the user would like*/
	bl printf

	ldr r0, string_ptr /*getting user input*/
	sub r1, fp, #20
	bl scanf
...
firstCompare:	
	ldrb r0, [fp, #-20]
	/*compare to see if it is a small pizza order*/
	cmp r0, #'s'
	bne else

	/*add one to small pizzas*/
	ldr r0, [fp, #-8] 
	add r0,r0, #1
	str r0, [fp, #-8]
	b ask
	...
	cmp r0, #'m'
	bne elseif
	/*add one to medium pizzas*/
	ldr r0, [fp, #-12]
	add r0, r0, #1
	str r0, [fp, #-12]
	b ask
elseif:
	/*if all nothing else, add one to large pizzas*/
	ldr r0, [fp, #-16]
	add r0, r0, #1
	str r0, [fp, #-16]
	b ask
...
ask:
	/*ask if they would like another pizza*/
	ldr r0, prompt7_ptr
	bl printf
